    ...
    public class Global
    {
        ...
        static public void saveToken(string _token, string _id)
        {
            ...
            try
            {
                ...
                var sql = "INSERT INTO tb_devices_admin (token, ID, REG_DATE, CONN_DATE) VALUES (@token,@id,@regdate,@conndate)";
                var mySqlCommand = new MySqlCommand(sql, connection);
                mySqlCommand.Parameters.AddWithValue("@token", _token);
                mySqlCommand.Parameters.AddWithValue("@id", _id);
                mySqlCommand.Parameters.AddWithValue("@regdate", nowTime);
                mySqlCommand.Parameters.AddWithValue("@conndate", nowTime);
        ...
        /// <param name="_token"></param>
        /// <param name="respJson"></param>
        static public void pushAlarm(string _seq, string _sender, string _recever, string _gubun, string _gateno, string _gatename, string _title, string _message, string _datetime, string _rtsp, ref JObject respJson)
        {
            string nowTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            var JHead = new JObject();
            var JBody = new JArray();
            ...
            try
            {
                ...
                var mySqlCommand = new MySqlCommand(sql, connection);
                mySqlCommand.Parameters.AddWithValue("@id", _recever);
                ...
                if (mySqlDataReader.Read())
                {
                    ...
                    sql = "SELECT * FROM tb_devices_admin WHERE ID = @id";
                    var mySqlCommand2 = new MySqlCommand(sql, connection);
                    mySqlCommand2.Parameters.AddWithValue("@id", _recever);
                    ...
                    while (mySqlDataReader2.Read())
                    {
                        ...
                        //푸시알림
                        SendToFirebaseMessagingServerAsync(_seq, token, _gubun, _gateno, _gatename, _title, _message, _datetime, _rtsp);
                        ...
                        var connection3 = new MySqlConnection(connectionString);
                        connection3.Open();
                        sql = "UPDATE tb_notice SET PUSH_YN='Y' WHERE SEQ = @seq";
                        var mySqlCommand3 = new MySqlCommand(sql, connection3);
                        mySqlCommand3.Parameters.AddWithValue("@seq", _seq);
                        if (mySqlCommand3.ExecuteNonQuery() == 1)
                        {
                            res = "SUCCESS";//성공
                            util.FileLogger("[ERROR][DataBase Notice] " + "푸시알림 성공 수신아이디:" + _recever);
                        }
                        else
                        {
                            res = "FALSE";
                            util.FileLogger("[ERROR][DataBase Notice] " + "푸시알림 실패 수신아이디:" + _recever);
                        }
                        connection3.Close();
                    }
                    mySqlDataReader2.Close();
        ...
        static public async Task SendToFirebaseMessagingServerAsync(String _seq, String _token, String _gubun, string _gateno, string _gatename, String _title, String _message, String _datetime, String _rtsp)
        {
            //await OnGetAsync(_token, _message);
            SendNotification(_seq, _token, _gubun, _gateno, _gatename, _title, _message, _datetime, _rtsp);
        }
        static public string SendNotification(string _seq, string deviceId, String _gubun, string _gateno, string _gatename, String _title, string _message, string _datetime, string _rtsp)
        {

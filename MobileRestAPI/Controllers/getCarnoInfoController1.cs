...
namespace MobileRestAPI.Controllers
{
    ...
    [Route("[controller]")]
    [ApiController]
    public class getCarnoInfoController : Controller
    {
        // GET api/values
        [HttpGet]
        public ActionResult<IEnumerable<string>> Get()
        {
            Global.util.FileLogger("[INFO][HttpGet getCarnoInfoController] ");
            ...
            if (auth == true)
            {
                ...
                //var carImage= request.Query["CarnoImage"];
                Global.util.FileLogger("[INFO][HttpGet getCarnoInfoController] : id:" + id + ", password:" + password + ", token:" + token + ", carno:" + Carno);

                Global.getCarnoInfo(id, password, token, Carno, ref respJson);
